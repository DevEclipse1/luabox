...
/**
 * @brief Draws a filled rectangle (optionally with rounded corners) on the sprite buffer.
 * 
 * This function draws a filled rectangle on the `sprite` buffer using the specified coordinates and color.
...
int DisplayRectFilled(lua_State* L_)
{
    ...
    if (r > 0)
        sprite.fillRoundRect(x1, y1, x2, y2, r, c);
    else
        sprite.fillRect(x1, y1, x2, y2, c);
